...
from swin_unet import SwinUnet2D
...
class AxisAlignedConvGaussian(nn.Module):
    ...
    def forward(self, input, segm=None):
        ...
        mu_log_sigma = torch.squeeze(mu_log_sigma, dim=2)  # (B, 2 * Latent_dim)

        mu = mu_log_sigma[:, :self.latent_dim]  # (B, latent_dim)
        log_sigma = mu_log_sigma[:, self.latent_dim:]  # (B,latent_dim)
        scale = F.softplus(log_sigma) + 1e-5
        # This is a multivariate normal with diagonal covariance matrix sigma
        # https://github.com/pytorch/pytorch/pull/11178
        dist = Independent(Normal(loc=mu, scale=scale), 1)  # (B,lantet_dim)
        return dist
...
class Probabilistic_Swin_Unet(nn.Module):
    """
    A probabilistic Swin UNet implementation.
    ...
    def __init__(self, input_channels=1, num_classes=1, num_filters=[16, 32, 64, 128, 192], latent_dim=6,
                 no_convs_fcomb=4, beta=10.0):
        super(Probabilistic_Swin_Unet, self).__init__()
        self.input_channels = input_channels
        self.num_classes = num_classes
        self.num_filters = num_filters
        self.latent_dim = latent_dim
